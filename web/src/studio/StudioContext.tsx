...
export function StudioProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    api.listImageGroups('seedance', videoModelId, 'video', controller.signal)
      .then(groups => {
        if (cancelled) return;
  ...
  const generateVideo = useCallback(
    (prompt: string, options?: GenerateVideoOptions) => {
      ...
      const runTask = async () => {
        try {
          updateTask({ status: 'processing' });
          let eligibleGroups = videoGroups;
          if (route.model !== videoModelId || !videoGroupsLoaded) {
            const discoveryController = new AbortController();
            const timeout = window.setTimeout(() => discoveryController.abort(), IMAGE_GROUP_DISCOVERY_TIMEOUT_MS);
            try {
              eligibleGroups = await api.listImageGroups('seedance', route.model, 'video', discoveryController.signal);
            } finally {
              window.clearTimeout(timeout);
            }
          }
  ...
  const retryBatchFailures = useCallback((uiId: string) => {
    ...
    void runRetry();
  }, [imageGroupsByModel, imageGroupStatusByModel, persistActiveProjectAssets, pollErrorMessages, t, tasks]);
